...
double forward_backward(const Graph& graph, const matrix<double>& gmmProbs,
  ...
  for (int frmIdx = 1; frmIdx <= frmCnt; ++frmIdx) {
    for (int stateIdx = 0; stateIdx < stateCnt; ++stateIdx) {
      ...
      for (int arcIdx = 0; arcIdx < arcCnt; ++arcIdx) {
        ...
        double forwLogProb = chart(frmIdx - 1, stateIdx).get_forw_log_prob();
        double transProb = arc.get_log_prob();
        double gmmProb = gmmProbs(frmIdx - 1, gmmIdx);
        double logProb = forwLogProb + transProb + gmmProb;
  ...
  int finalStateCnt = graph.get_final_state_list(finalStates);
  for (int stateIdx = 0; stateIdx < finalStateCnt; ++stateIdx) {
    chart(frmCnt, finalStates[stateIdx]).set_back_log_prob(0);
  }
  ...
  for (int frmIdx = frmCnt - 1; frmIdx >= 0; --frmIdx) {
    for (int stateIdx = 0; stateIdx < stateCnt; ++stateIdx) {
      ...
      for (int arcIdx = 0; arcIdx < arcCnt; ++arcIdx) {
        ...
        double backLogProb = chart(frmIdx + 1, dstState).get_back_log_prob();
        double logProb = backLogProb + transProb + gmmprob;
        
        logProb = add_log_probs(vector<double> {logProb, chart(frmIdx, stateIdx).get_back_log_prob()});
        chart(frmIdx, stateIdx).set_back_log_prob(logProb);
      }
    }
  }
  ...
  for (int frmIdx = frmCnt; frmIdx > 0; --frmIdx) {
    for (int stateIdx = 0; stateIdx < stateCnt; ++stateIdx) {
      ...
      for (int arcIdx = 0; arcIdx < arcCnt; ++arcIdx) {
        ...
        double logProb = forwLogProb + transProb + gmmprob + backLogProb;
        
        double arcPosterior = exp(logProb - uttLogProb);
        gmmCountList.push_back(GmmCount(arc.get_gmm(), frmIdx - 1, arcPosterior));
      }
    }
  }
